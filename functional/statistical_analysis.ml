...
(* Definimos los tipos de datos para nuestras estructuras de datos *)
type student = {
  id: string;
  name: string;
  program: string;
...
let parse_course json =
  {
    ...
    student_grades = json |> member "student_grades" |> to_assoc |> List.map (fun (k, v) -> (k, to_float v));
  }

let parse_evaluation json =
  let student_scores =
    json |> member "student_scores" |> to_assoc |> List.map (fun (student_id, v) ->
      let by_topic = v |> member "by_topic" |> to_assoc |> List.map (fun (tid, score) -> (tid, to_float score)) in
      (student_id, by_topic)
    )
  in
  {
...
let collect_topic_course_scores dataset =
  List.fold_left (fun acc eval ->
    List.fold_left (fun acc topic_id ->
      let scores =
        List.filter_map (fun (student_id, by_topic) ->
...
(* 2. Student performance trends (over time, by date) *)
let calculate_student_trends dataset =
  ...
  in
  StringMap.mapi (fun student_id scores ->
...
(* 3. Critical performance points (difficulty/failure rate by topic) *)
let calculate_critical_points dataset =
  let topic_scores =
    List.fold_left (fun acc eval ->
      List.fold_left (fun acc topic_id ->
        let scores =
          List.filter_map (fun (_, by_topic) ->
            match List.assoc_opt topic_id by_topic with Some s -> Some s | None -> None
          ) eval.student_scores
        in
  ...
  in
  StringMap.mapi (fun topic_id scores ->
    let n = float_of_int (List.length scores) in
    let mean = List.fold_left (+.) 0. scores /. n in
...
(* 4. Topic correlations (correlation between topics) *)
  ...
  List.fold_left (fun acc t1 ->
    List.fold_left (fun acc t2 ->
      if t1 < t2 then
        ...
        let xs = List.take xs min_len in
        let ys = List.take ys min_len in
...
let analyze_data json_file =
  ...
  let output_json = `Assoc [
    ("topic_performance", `Assoc (TopicCourseMap.bindings topic_performance));
